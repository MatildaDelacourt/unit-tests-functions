...
const store = {
  ...
  getExpensiveItems(maxPrice) {
    const expItem = inventory.filter((item) => item.price >= maxPrice);
   ...
   */
  getStoreItems() {
    const newarr = inventory.map((item) => item.name);
    return newarr;
  },
  /**
  ...
  isItemInStore(itemName) {
    const search = inventory.some((item) => itemName === item.name);
    return search;
  },
  /**
  ...
  getItemPrice(itemName) {
    if (this.isItemInStore(itemName)) {
      const price = inventory.find((item) => itemName === item.name);
      return price.price;
    }
    return -1;
  },
  ...
  getItemQuantity(itemName) {
    if (this.isItemInStore(itemName)) {
      const quantity = inventory.find((item) => itemName === item.name);
